...
def initialize_deep_models(args):
	start = time.time()
	model = FaceModel(args)
	#if args.verbose:
	print("Loading the model took {} seconds.".format(
	    time.time() - start))
	return model

def get_deep_features(lX, model):
	L = len(lX)
	lsfeats = []
	start = time.time()	
	for i in range(L):
		alignedFace = model.get_input(lX[i])
		if alignedFace is None:
		    print("Unable to align image...")
		rep = model.get_feature(alignedFace)
		lsfeats.append(rep)
		#print(i)
	npfeats = np.array(lsfeats)
	print("MXNet feature extraction took "+str(time.time() - start) + " seconds for " + str(L) +" faces.")

	return npfeats

def get_deep_feature(X, model):

	alignedFace = model.get_input(X)
	if alignedFace is None:
	    print("Unable to align image...")
	rep = model.get_feature(alignedFace)
	return np.array(rep)
